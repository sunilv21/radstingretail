...
## Test results (2026-06-16 verification pass)

### Executable algorithm tests — `node server/scripts/algo-tests.mjs`
**54 / 54 PASS.** No database required (pure logic).

| Suite | Cases | Result |
|---|---|---|
| GST per-item tax (excl/incl, intra/inter, zero-rate, %/flat discount) | 14 | ✅ all pass |
| GST cart totals (incl-250→₹250, excl→₹280, qty×3, mixed, inter-state; `grandTotal == Σ line.totalAmount`, chain balances) | 15 | ✅ all pass |
| Ledger `postVoucher` guard (rejects unbalanced / negative / single-entry) | 3 | ✅ all pass |
| RBAC matrix escalation (cashier denials, legit allows, manager/accountant/CA/super_admin) | 19 | ✅ all pass |
| Sequence allocator (no duplicates across concurrent workers, legacy seed) | 3 | ✅ all pass |

Plus a focused RBAC re-check after the read-style-POST fix: **9/9** (CA/manager allowed on reconcile/bank-rec/preview reads; cashier still denied).

### Static verification (4 parallel re-audits)
- **API/CRUD/RBAC:** full per-route table produced; `enforceResource` resource names all valid (no mass-deny bug); deliberately-ungated routers (pos, reports, hsn) keep cashier billing working; found + fixed 3 read-style-POST over-restrictions.
- **Frontend buttons/forms:** ~60 buttons/forms tabulated; core financial flows (sale, GRN, payment, return, voucher) all gate `submitting` + refetch correctly; 3 P2 double-submit gaps + minor swallowed-error notes (api.ts normalizes network errors, so low impact).
- **Engine/service regression:** all 5 session changes confirmed correct; found + fixed the stale `store.stateCode` regression; double-entry balance re-confirmed for every posting type.
- **GST inclusive parity:** `lib/billing-local.ts` confirmed in exact lockstep with `server/engines/gst.engine.js`.

### Build gates
`npx tsc --noEmit` ✅ · `npm run lint` ✅ · `npm run build` ✅ · `server/app.js` imports without connecting ✅

### What is NOT covered (needs a disposable QA DB + cluster)
Live multi-document transactions (the ledger postings call `resolveAccount` → DB, so `recordSale`/GRN balance is asserted by code review + the `postVoucher` unit guard, not an integration run), concurrent-cashier invoice uniqueness under real Mongo, kill-mid-transaction, 1000-item cart latency, load test to find the real per-store ceiling, live e-invoice/GSP/payment webhooks, and browser click-through. The manual suite for these is in [test-cases.md §33](test-cases.md) and the integrity gate is [server/scripts/integrity-scan.js](../server/scripts/integrity-scan.js).

*Verification pass: executable tests + 4 parallel static re-audits; 2 self-introduced regressions found and fixed; all original P0/financial-P1 fixes confirmed.*
